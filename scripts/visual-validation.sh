...
case "${IDENTIFIER}" in
  ...
  api-token-settings)
    ...
    ;;
  "")
    echo "Usage: ./scripts/visual-validation.sh {incoming-invoice-inbox|rolling-year-period-default|customer-payment-notes-billing-defaults|bulk-toolbar-spacing|api-token-settings}" >&2
    exit 2
    ;;
  *)
